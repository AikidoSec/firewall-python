...
def test_replace_numbers():
    ...
    assert build_route_from_url("http://localhost/posts/3/") == "/posts/:number"
    assert build_route_from_url("http://localhost/posts/3/comments/10") == "/posts/:number/comments/:number"
    assert build_route_from_url("/blog/2023/05/great-article") == "/blog/:number/:number/great-article"

def test_replace_dates():
    assert build_route_from_url("/posts/2023-05-01") == "/posts/:date"
    assert build_route_from_url("/posts/2023-05-01/") == "/posts/:date"
    assert build_route_from_url("/posts/2023-05-01/comments/2023-05-01") == "/posts/:date/comments/:date"
...
def test_ignore_invalid_uuids():
    assert build_route_from_url("/posts/00000000-0000-1000-6000-000000000000") == "/posts/00000000-0000-1000-6000-000000000000"
...
def test_replace_ip_addresses():
    ...
    assert build_route_from_url("/block/2001:2:ffff:ffff:ffff:ffff:ffff:ffff") == "/block/:ip"
...
def test_replace_secrets():
    assert build_route_from_url("/confirm/CnJ4DunhYfv2db6T1FRfciRBHtlNKOYrjoz") == "/confirm/:secret"
