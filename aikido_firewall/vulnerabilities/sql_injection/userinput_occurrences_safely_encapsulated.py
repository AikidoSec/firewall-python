...
def userinput_occurrences_safely_encapsulated(query, user_input):
    ...
    for segment in segments_in_between:
        ...
        quote_char = next((char for char in SQL_STRING_CHARS if char == char_before_user_input), None)

        for char in ['"', "'"]:
            if not quote_char and input_str.startswith(char) and current_segment[-2:] == f"{char}\\" and char_after_user_input == char:
                quote_char = char
