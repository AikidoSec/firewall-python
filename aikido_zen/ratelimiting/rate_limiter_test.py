...
def test_allow_requests_within_limit(rate_limiter):
    key = "user1"
    for i in range(5):
        assert rate_limiter.is_allowed(key, 5000000, 5)[
            "allowed"
...
def test_deny_requests_exceeding_limit(rate_limiter):
    ...
    assert not rate_limiter.is_allowed(key, 500, 5)[
        "allowed"
...
def test_clear_old_entries(rate_limiter):
    ...
    assert rate_limiter.is_allowed(key, 500, 5)[
        "allowed"
...
def test_multiple_keys(rate_limiter):
    ...
    for i in range(5):
        assert rate_limiter.is_allowed(key1, 500, 5)[
            "allowed"
        ], f"Request {i + 1} for key1 should be allowed"
    for i in range(5):
        assert rate_limiter.is_allowed(key2, 500, 5)[
            "allowed"
        ], f"Request {i + 1} for key2 should be allowed"

    assert not rate_limiter.is_allowed(key1, 500, 5)[
        "allowed"
    ], "Request 6 for key1 should not be allowed"
    assert not rate_limiter.is_allowed(key2, 500, 5)[
        "allowed"
...
def test_different_window_sizes(rate_limiter):
    ...
    for i in range(5):
        assert rate_limiter.is_allowed(key, different_window_size, 5)[
            "allowed"
        ], f"Request {i + 1} should be allowed"
    assert not rate_limiter.is_allowed(key, different_window_size, 5)[
        "allowed"
    ], "Request 6 should not be allowed"
...
def test_sliding_window_with_burst_requests(rate_limiter):
    ...
    # Allow 5 requests in a 1-second window
    for i in range(5):
        assert rate_limiter.is_allowed(key, window_size_ms, 5)[
            "allowed"
        ], f"Request {i + 1} should be allowed"
    ...
    assert not rate_limiter.is_allowed(key, window_size_ms, 5)[
        "allowed"
...
def test_retry_after_seconds_when_rate_limited(rate_limiter):
    key = "user18"
    window_size_ms = 1000
    max_requests = 2
    limiter = RateLimiter(10, window_size_ms)

    assert limiter.is_allowed(key, window_size_ms, max_requests) == {"allowed": True}
    assert limiter.is_allowed(key, window_size_ms, max_requests) == {"allowed": True}

    time.sleep(0.3)

    result = limiter.is_allowed(key, window_size_ms, max_requests)
